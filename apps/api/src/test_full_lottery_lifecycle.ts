async function runFullLotteryLifecycleTest() {
  ...
  const custData: any = await custLoginRes.json();
  ...
  const livePreData: any = await livePreRes.json();
  ...
  const spinData: any = await spinRes.json();
  if (!spinRes.ok) throw new Error('Live spin failed: ' + JSON.stringify(spinData));
  ...
  const livePostData: any = await livePostRes.json();
