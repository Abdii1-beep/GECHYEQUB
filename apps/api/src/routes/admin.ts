...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const calculatedRevenue = totalRevenue.reduce(
        (sum: number, ticket: any) => sum + (ticket.LotteryCampaign?.ticketPrice || 0),
        0
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const totalRevenue = revenueData.reduce(
        (sum, ticket) => sum + (ticket.LotteryCampaign?.ticketPrice || 0),
        0
      );
...
router.get(
  "/kyc-queue",
  authenticateToken,
  authorizeRoles("KYC_OFFICER", "SUPER_ADMIN", "LOTTERY_MANAGER"),
  async (req: Request, res: Response) => {
    try {
    ...
    try {
      ...
      const users = await prisma.user.findMany({
        select: {
          ...
          kycRecord: { select: { idVerificationStatus: true } },
        },
        orderBy: { createdAt: "desc" },
      });

      // Fetch tickets for these users
      const tickets = await prisma.ticket.findMany({
        where: { customerId: { in: users.map(u => u.id) } },
        select: { id: true, purchaseId: true, status: true, customerId: true },
      });

      // Group tickets by user id
      const ticketsByUser: Record<string, typeof tickets> = {};
      tickets.forEach(t => {
        if (!ticketsByUser[t.customerId]) ticketsByUser[t.customerId] = [];
        ticketsByUser[t.customerId].push(t);
      });

      const formattedUsers = users.map(u => ({
        ...u,
        tickets: ticketsByUser[u.id] || [],
        customerProfile: u.customerProfile
          ? { ...u.customerProfile, kycStatus: u.kycRecord?.idVerificationStatus || "NOT_SUBMITTED" }
          : null,
      }));
