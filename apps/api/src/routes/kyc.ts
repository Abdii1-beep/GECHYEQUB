  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await createAuditLog(
        ...
        {
          status: "PENDING",
          submittedFields: Object.keys(req.body).filter(
...
router.get(
  "/pending",
  authenticateToken,
  authorizeRoles("KYC_OFFICER", "SUPER_ADMIN", "AUDITOR"),
  async (req: AuthRequest, res: Response) => {
    try {
...
router.put(
  "/:id/approve",
  authenticateToken,
  authorizeRoles("KYC_OFFICER", "SUPER_ADMIN"),
  async (req: AuthRequest, res: Response) => {
    try {
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await createAuditLog(
        ...
        {
          status: "PENDING",
          resubmittedFields: Object.keys(req.body).filter(
            (k) => req.body[k] !== undefined && req.body[k] !== null
          ),
        }
      );
