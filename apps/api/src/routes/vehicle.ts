...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    if (!includeAll) where.status = "AVAILABLE";
    ...
    try {
      ...
      if (existingVehicle.campaignId) {
        ...
        if (campaignWithTickets) {
          ...
          const hasCoreChange = requested.some(k => coreSpecFields.includes(k) && req.body[k] !== (existingVehicle as any)[k]);
          if (hasCoreChange) {
            return res.status(400).json({
      ...
      if (vinChassisNumber !== undefined) updateData.vinChassisNumber = vinChassisNumber;
      if (registrationInfo !== undefined) updateData.registrationInfo = registrationInfo;
      if (vehicleCondition !== undefined) updateData.vehicleCondition = vehicleCondition;
      if (location !== undefined) updateData.location = location;
      if (declaredValue !== undefined) updateData.declaredValue = declaredValue;
      if (status !== undefined) updateData.status = status;
      if (images !== undefined) {
        let normalizedImgs: string[] = [];
        if (Array.isArray(images)) {
          normalizedImgs = images.map(i => (typeof i === 'string' ? i.trim() : String(i))).filter(Boolean);
        } else if (typeof images === 'string') {
          try {
            const p = JSON.parse(images);
            if (Array.isArray(p)) normalizedImgs = p.map(i => (typeof i === 'string' ? i.trim() : String(i))).filter(Boolean);
          } catch {
