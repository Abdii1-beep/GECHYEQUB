...
import { PrismaClient, Role, PaymentProvider as PaymentProviderEnum } from "@prisma/client";
import { PaymentProviderFactory } from "../services/payment/PaymentProviderFactory";
import { PaymentRequest, PaymentVerificationRequest, RefundRequest } from "../services/payment/PaymentProvider";
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const providerName = (provider as PaymentProviderEnum) || PaymentProviderEnum.CBE;
      ...
      await createAuditLog(
        ...
        { orderStatus: "PENDING", providerTxnId: result.providerReference }
      );
    ...
    try {
      ...
      if (!result.success || !result.verified) {
        ...
        await createAuditLog(
          ...
          "FAILED",
          beforeValue,
          { status: "FAILED", verificationResult: result }
        );
      ...
      await createAuditLog(
        ...
        "SUCCESS",
        beforeValue,
        { status: "SUCCESS", verificationResult: result, ticketsUpdated: tickets.length }
      );
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await createAuditLog(
        ...
        "SUCCESS",
        beforeValue,
        { status: "REFUNDED", refundId: result.refundId, ticketsUpdated: tickets.length }
      );
