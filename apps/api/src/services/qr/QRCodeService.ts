...
class QRCodeService {
   ...
   * @returns QR code data URL
   */
  static async generateVerificationQRCode(ticketNumber: string): Promise<string> {
    try {
      const payload = {
        tn: ticketNumber,
        type: "VERIFY",
  ...
  static async generateBatchQRCodes(
    tickets: Array<{ id: string; ticketNumber: string; campaignId: string }>
  ): Promise<Array<{ ticketId: string; qrCode: string }>> {
    const results = await Promise.all(
      tickets.map(async (ticket) => {
        const qrCode = await this.generateTicketQRCode(
          ticket.id,
          ticket.ticketNumber,
          ticket.campaignId
        );
   ...
   */
  static async generateTicketQRCodeSVG(
    ticketId: string,
    ticketNumber: string,
    campaignId: string
  ): Promise<string> {
