 ...
 */
export class BIRRProvider extends PaymentProviderInterface {
  ...
  constructor(config: Record<string, any>) {
    super(PaymentProviderEnum.BIRR, config);
    
    this.apiKey = config.apiKey || "";
    this.merchantId = config.merchantId || "";
    this.secretKey = config.secretKey || "";
    this.apiUrl = config.apiUrl || "https://api.birr.com.et/v1";
  }
    ...
    try {
      if (!this.validateConfig()) {
        return {
          success: false,
          status: "FAILED",
          message: "BIRR provider configuration is invalid",
        };
      }
      ...
      // Make API request to BIRR payment gateway
      const response = await this.makeBIRRRequest("/payments/initiate", {
        ...payload,
        signature,
      });
      ...
      console.error("BIRR initiate payment error:", error);
      return {
        success: false,
        status: "FAILED",
  ...
  async verifyPayment(request: PaymentVerificationRequest): Promise<PaymentVerificationResponse> {
    try {
      ...
      // Make verification request to BIRR
      const response = await this.makeBIRRRequest("/payments/verify", {
        ...payload,
        signature,
      });
      ...
      console.error("BIRR verify payment error:", error);
      return {
        success: false,
        status: "FAILED",
        verified: false,
  ...
  async processRefund(request: RefundRequest): Promise<RefundResponse> {
    try {
      ...
      const response = await this.makeBIRRRequest("/payments/refund", {
        ...payload,
        signature,
      });
      ...
      console.error("BIRR process refund error:", error);
      return {
        success: false,
        status: "FAILED",
  ...
  private generateTransactionRef(orderId: string): string {
    ...
    return `BIRR-${orderId}-${timestamp}-${random}`.toUpperCase();
  }
  ...
  private async makeBIRRRequest(endpoint: string, payload: any): Promise<any> {
    // In production, this would make actual HTTP requests to BIRR API
    // For now, return mock response
    console.log(`BIRR API Request: ${this.apiUrl}${endpoint}`, payload);
    
    // Mock response for development
    return {
      success: true,
      payment_id: `BIRR-${Date.now()}`,
      payment_url: `https://birr-payment-gateway.com/pay/${payload.transaction_ref}`,
