...
export class SecureDrawEngine {
   ...
   */
  private static generateSecureSeed(): string {
   ...
   */
  static async prepareDraw(campaignId: string, preparedBy: string): Promise<{
    success: boolean;
    drawId?: string;
    commitmentHash?: string;
   ...
   */
  static async executeDraw(drawId: string, executedBy: string): Promise<{
    success: boolean;
    results?: any[];
    message?: string;
  }> {
    try {
      ...
      const isCommitmentValid = this.verifyCommitmentHash(
        draw.commitmentHash,
        draw.seed || "",
        draw.eligibleTickets.map((et: any) => et.ticketId),
        draw.createdAt
      );
   ...
   */
  static async verifyDraw(drawId: string): Promise<{
    success: boolean;
    isValid?: boolean;
    details?: any;
    message?: string;
    ...
    try {
      ...
      const resultTicketIds = draw.results.map((r: any) => r.ticketId);
      const eligibleTicketIds = draw.eligibleTickets.map((et: any) => et.ticketId);
      const allResultsFromEligible = resultTicketIds.every((id: string) => 
        eligibleTicketIds.includes(id)
      );
   ...
   */
  static async getTransparencyData(drawId: string): Promise<{
    success: boolean;
    data?: any;
    message?: string;
  }> {
