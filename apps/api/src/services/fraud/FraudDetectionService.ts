...
class FraudDetectionService {
   ...
   */
  static async calculateRiskScore(userId: string): Promise<FraudRiskScore> {
  ...
  static async detectSuspiciousActivity(userId: string): Promise<SuspiciousActivity[]> {
    ...
    const purchasesAtNight = oddHourPurchases.filter(
      (t) => t.createdAt.getHours() >= 2 && t.createdAt.getHours() <= 5
    );
