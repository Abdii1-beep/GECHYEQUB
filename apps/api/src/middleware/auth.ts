...
export const requireKYC = (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  const adminRoles: Role[] = ["SUPER_ADMIN", "LOTTERY_MANAGER", "FINANCE_OFFICER", "KYC_OFFICER", "DRAW_OFFICER", "AUDITOR", "CUSTOMER_SUPPORT", "CONTENT_MANAGER"];
...
export const generateTokens = (userId: string, email: string, role: Role) => {
  ...
  const refreshToken = jwt.sign(
    ...
    process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET || "fallback_refresh_secret_2026",
    { expiresIn: "7d" }
  );
