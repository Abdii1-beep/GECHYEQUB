...
router.get("/financial", authenticateToken, authorizeRoles("FINANCE_OFFICER", "SUPER_ADMIN"), async (req, res) => {
    try {
        ...
        res.json({
            ...
            paymentStatusBreakdown: payments.reduce((acc, p) => {
...
router.get("/tickets", authenticateToken, authorizeRoles("LOTTERY_MANAGER", "SUPER_ADMIN"), async (req, res) => {
    try {
...
// @access  Private (LOTTERY_MANAGER+, SUPER_ADMIN)
router.get("/customers", authenticateToken, authorizeRoles("LOTTERY_MANAGER", "SUPER_ADMIN"), async (req, res) => {
    try {
        ...
        res.json({
            summary: {
                ...
                totalVerified: users.filter((u) => u.kycRecord?.idVerificationStatus === "VERIFIED").length,
                activeCustomers,
                kycStats,
            },
            trends: {
...
router.get("/draws", authenticateToken, authorizeRoles("DRAW_OFFICER", "SUPER_ADMIN"), async (req, res) => {
    try {
        const { startDate, endDate, campaignId } = req.query;
        const where = {};
        if (startDate && endDate) {
...
router.get("/overview", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const now = new Date();
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        // Get key metrics
        const [totalUsers, activeCampaigns, totalTickets, totalDraws, recentPayments, recentTickets,] = await Promise.all([
            prisma.user.count(),
...
router.get("/export", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER", "FINANCE_OFFICER"), async (req, res) => {
    try {
