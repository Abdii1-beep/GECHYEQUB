...
router.post("/prepare", authenticateToken, authorizeRoles("DRAW_OFFICER", "SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const { campaignId } = req.body;
        if (!campaignId) {
            return res.status(400).json({ message: "Campaign ID required" });
        }
        const result = await SecureDrawEngine.prepareDraw(campaignId, req.userId);
        if (!result.success) {
            return res.status(400).json({ message: result.message });
        }
        await createAuditLog(req.userId, req.role, "PREPARE_DRAW", "Draw", result.drawId, req, "SUCCESS", { campaignId }, { drawId: result.drawId, commitmentHash: result.commitmentHash });
        res.json({
...
// @access  Private (DRAW_OFFICER+)
router.post("/execute", authenticateToken, authorizeRoles("DRAW_OFFICER", "SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        ...
        await createAuditLog(req.userId, req.role, "EXECUTE_DRAW", "Draw", drawId, req, "SUCCESS", { state: "PREPARED" }, { state: "COMPLETED", results: result.results });
        res.json({
...
router.post("/verify", authenticateToken, authorizeRoles("AUDITOR", "SUPER_ADMIN", "DRAW_OFFICER", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const { drawId } = req.body;
        if (!drawId) {
            return res.status(400).json({ message: "Draw ID required" });
        ...
        await createAuditLog(req.userId, req.role, "VERIFY_DRAW", "Draw", drawId, req, result.isValid ? "SUCCESS" : "FAILED", {}, { isValid: result.isValid, details: result.details });
        res.json({
...
// @access  Private (DRAW_OFFICER+)
router.get("/", authenticateToken, authorizeRoles("DRAW_OFFICER", "SUPER_ADMIN", "LOTTERY_MANAGER", "AUDITOR"), async (req, res) => {
    try {
...
const extractOptionalUser = (req) => {
    try {
        ...
        if (!token)
            return null;
        const decoded = jwt.verify(token, process.env.JWT_SECRET || "ethiopian_lottery_jwt_secret_2026_secure_key");
...
router.get("/live/:campaignId", async (req, res) => {
    try {
...
router.post("/live/schedule", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER", "DRAW_OFFICER"), async (req, res) => {
    try {
        ...
        liveSessions.set(campaignId, session);
        await createAuditLog(req.userId, req.role, "SCHEDULE_LIVE_DRAW", "LotteryCampaign", campaignId, req, "SUCCESS", null, { scheduledDate: date.toISOString() });
        res.json({
...
// @access  Private (SUPER_ADMIN, LOTTERY_MANAGER, DRAW_OFFICER)
router.post("/live/spin", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER", "DRAW_OFFICER"), async (req, res) => {
    try {
        const { campaignId } = req.body;
        if (!campaignId) {
        ...
        liveSessions.set(campaignId, session);
        await createAuditLog(req.userId, req.role, "START_LIVE_SPIN", "Draw", draw?.id || campaignId, req, "SUCCESS", null, { winner: winnerInfo, spinStartedAt: now });
        res.json({
...
router.post("/live/reset", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const { campaignId } = req.body;
        if (!campaignId) {
            return res.status(400).json({ message: "Campaign ID required" });
