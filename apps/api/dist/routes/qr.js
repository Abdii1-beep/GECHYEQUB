...
router.get("/ticket/:ticketId", authenticateToken, async (req, res) => {
    try {
        ...
        // Generate QR code
        const qrCode = await QRCodeService.generateTicketQRCode(ticket.id, ticket.purchaseId, // Using purchaseId as ticket identifier
...
// @access  Private
router.get("/verify/:purchaseId", authenticateToken, async (req, res) => {
    try {
...
// @access  Private
router.get("/ticket/:ticketId/svg", authenticateToken, async (req, res) => {
    try {
        const { ticketId } = req.params;
        const ticket = await prisma.ticket.findUnique({
            where: { id: ticketId },
        ...
        const qrCodeSVG = await QRCodeService.generateTicketQRCodeSVG(ticket.id, ticket.purchaseId, ticket.LotteryCampaign?.id || ticket.campaignId);
...
router.post("/batch", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
...
// @access  Private
router.post("/styled", authenticateToken, async (req, res) => {
    try {
        ...
        const qrCode = await QRCodeService.generateStyledQRCode(ticket.id, ticket.purchaseId, ticket.LotteryCampaign?.id || ticket.campaignId, { color, backgroundColor, width });
        res.json({
            ticketId: ticket.id,
            ticketIdentifier: ticket.purchaseId,
            qrCode,
