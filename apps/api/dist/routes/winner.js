...
router.get("/", authenticateToken, authorizeRoles("AUDITOR", "FINANCE_OFFICER", "SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
...
// @access  Private
router.get("/draw/:drawId", authenticateToken, async (req, res) => {
    try {
        const winners = await prisma.winner.findMany({
...
router.put("/:id/verify", authenticateToken, authorizeRoles("DRAW_OFFICER", "AUDITOR", "SUPER_ADMIN"), async (req, res) => {
    try {
        const { verificationStatus, notificationStatus, claimStatus, rejectionReason } = req.body;
...
router.put("/:id/deliver", authenticateToken, authorizeRoles("DRAW_OFFICER", "SUPER_ADMIN"), async (req, res) => {
    try {
        const winnerId = req.params.id;
        const updatedWinner = await prisma.winner.update({
            where: { id: winnerId },
