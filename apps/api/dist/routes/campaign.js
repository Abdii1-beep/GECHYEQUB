...
router.post("/", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const { name, description, systemName, ticketPrice, maxTickets, startDate, endDate, drawDate, eligibilityRules, } = req.body;
        // Validation
        ...
        // Create audit log
        await createAuditLog(req.userId, req.role, "CREATE_CAMPAIGN", "LotteryCampaign", campaign.id, req, "SUCCESS", null, campaignResult);
        res.status(201).json({
...
router.put("/:id", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        ...
        if (req.body.systemName !== undefined)
            updateData.systemName = req.body.systemName;
        if (req.body.ticketPrice !== undefined)
            updateData.ticketPrice = Number(req.body.ticketPrice);
        if (req.body.maxTickets !== undefined)
            updateData.maxTickets = Number(req.body.maxTickets);
        if (req.body.startDate !== undefined)
            updateData.startDate = new Date(req.body.startDate);
        if (req.body.endDate !== undefined)
            updateData.endDate = new Date(req.body.endDate);
        if (req.body.drawDate !== undefined)
            updateData.drawDate = req.body.drawDate ? new Date(req.body.drawDate) : null;
        if (req.body.eligibilityRules !== undefined)
            updateData.eligibilityRules = req.body.eligibilityRules;
        ...
        if (req.body.vehicle) {
            ...
            if (existingCampaign.vehicles && existingCampaign.vehicles.length > 0) {
                ...
                if (v.model !== undefined)
                    vUpdate.model = v.model;
                if (v.year !== undefined)
                    vUpdate.year = Number(v.year) || existingCampaign.vehicles[0].year;
                if (v.color !== undefined)
                    vUpdate.color = v.color;
                if (v.engineInfo !== undefined)
                    vUpdate.engineInfo = v.engineInfo;
                if (v.transmission !== undefined)
                    vUpdate.transmission = v.transmission;
                if (v.fuelType !== undefined)
                    vUpdate.fuelType = v.fuelType;
                if (v.vinChassisNumber !== undefined)
                    vUpdate.vinChassisNumber = v.vinChassisNumber;
                if (v.declaredValue !== undefined)
                    vUpdate.declaredValue = Number(v.declaredValue) || 0;
                if (v.vehicleCondition !== undefined)
                    vUpdate.vehicleCondition = v.vehicleCondition;
                if (v.location !== undefined)
                    vUpdate.location = v.location;
        ...
        // Create audit log
        await createAuditLog(req.userId, req.role, "UPDATE_CAMPAIGN", "LotteryCampaign", updatedCampaign.id, req, "SUCCESS", beforeValue, campaignResult);
        res.json({
...
// @access  Private (LOTTERY_MANAGER+)
router.delete("/:id", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const campaign = await prisma.lotteryCampaign.findUnique({
            where: { id: req.params.id },
        });
        ...
        if (paidTickets > 0) {
            ...
            await createAuditLog(req.userId, req.role, "CANCEL_CAMPAIGN", "LotteryCampaign", campaign.id, req, "SUCCESS", campaign, updatedCampaign);
            return res.json({
        ...
        await createAuditLog(req.userId, req.role, "DELETE_CAMPAIGN", "LotteryCampaign", campaign.id, req, "SUCCESS", campaign, null);
...
// @access  Private (LOTTERY_MANAGER+)
router.post("/:id/publish", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER"), async (req, res) => {
    try {
        const campaign = await prisma.lotteryCampaign.findUnique({
            where: { id: req.params.id },
        });
        ...
        await createAuditLog(req.userId, req.role, "PUBLISH_CAMPAIGN", "LotteryCampaign", campaign.id, req, "SUCCESS", campaign, updatedCampaign);
        res.json({
