...
router.post("/send", authenticateToken, authorizeRoles("SUPER_ADMIN", "LOTTERY_MANAGER", "CUSTOMER_SUPPORT"), async (req, res) => {
    try {
...
router.post("/broadcast", authenticateToken, authorizeRoles("SUPER_ADMIN"), async (req, res) => {
    try {
        ...
        const notificationPromises = users.map((user) => createNotification(user.id, type, title, message, data));
